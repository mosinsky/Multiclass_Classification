...
def classify_text(input_text):
    ...
    preprocessed_text = preprocess_text(cleaned_text)  # Use the preprocessing steps you need
    ...
    test = cv.transform([text_without_pos_tags])  # Use the processed text
    test_tfidf = tfidf_t.transform(test)

    # Make prediction using the loaded model
    prediction = model.predict(test_tfidf)[0]

    return text_without_pos_tags, prediction

# Create a Gradio interface with an API endpoint
iface = gr.Interface(
    # fn=classify_text,
    fn=lambda x: (translate_to_english(x),) + classify_text(x),
    inputs=gr.Textbox(placeholder="Enter text...", label = "Complaint"),

    outputs=[gr.Textbox(placeholder="Translated Text", label = "Preprocessed_text"),
              gr.Textbox(placeholder="leaned Text", label = "Prediction"),
              gr.Textbox(placeholder="Prediction", label = "Prediction")]
)
iface.launch(share=True)

# I tried to make a transaction at a supermarket retail store, using my chase \
